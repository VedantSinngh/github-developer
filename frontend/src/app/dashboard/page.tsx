  ...
  return (
    <div className="min-h-screen bg-canvas text-ink p-12 font-sans relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        ...
        <div className="bg-surface-card border border-hairline rounded-xl overflow-hidden shadow-soft">
          <table className="w-full text-left text-sm">
            <thead className="bg-canvas-soft border-b border-hairline text-[11px] uppercase tracking-[0.96px] text-muted">
              <tr>
                <th className="p-6 font-semibold">Candidate</th>
                <th className="p-6 font-semibold">Repository</th>
                <th className="p-6 font-semibold">Status</th>
                <th className="p-6 font-semibold">Final Score</th>
                <th className="p-6 text-right font-semibold">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-hairline-soft">
              {filtered.map((ev) => (
                <tr key={ev.id} className="hover:bg-canvas-soft/60 transition">
                  <td className="p-6 font-serif font-normal text-lg text-ink">{ev.candidate}</td>
                  <td className="p-6 font-sans text-xs text-body font-mono">{ev.repo}</td>
                  <td className="p-6">
                    <StatusBadge status={ev.status} />
                  </td>
                  <td className="p-6 font-serif font-light text-2xl text-ink">
